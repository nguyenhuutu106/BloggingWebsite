        ...
        <div class="container">
          ...
          <div class="post-main">
            ...
            <div class="wrapper__breadcrumbs">
              <div class="breadcrumbs">
                <ul>
                  ...
                  <li><a href="<?=ROOT_URL?>categories.php" class="a__breadscrumbs">
                  ...
                  <?php
                  $result = $user->getCategoryName($id);
                  $row = $result->fetch_assoc();
                  $categoryName = $row['CategoryName'];
                  $categoryContent = $row['CategoryContent'];
                  ?>
                  <p><?=$categoryName ?></p>
                </a></li>
                </ul>
            ...
            <h2 class="headline headline-2 section-title">
              <span class="span"><?=$categoryContent ?></span>
            </h2>
            ...
            $result = $user->getPostByCategory($id, $startRow, $pageSize); 
            if($result->num_rows == 0) { ?>
              <h1 class="headline headline-1 section-title">
                <span class="span">Hiện vẫn chưa có bài đăng nào về thể loại này</span>
              </h1>
              <?php } else { ?>
              <ul class="grid-list">
            ...
            <?php } ?>
            <?php
            $totalRecord = $user->getPostByCategoryRowCount($id);
            ...
            if($totalRecord > 7) {  ?>
              ...
              <?php if($pageNum > 1) { ?>
                <a href='<?=ROOT_URL?>categories_post.php?id=<?=$id?>'  class="pagination-btn" aria-label="previous page">
                <i class="fa-solid fa-backward"></i>
                </a>
  
                <a href='<?=ROOT_URL?>categories_post.php?id=<?=$id?>&pageNum=<?=$pagePrev?>' class="pagination-btn" aria-label="previous page">
                <i class="fa-solid fa-arrow-left"></i>
                </a>
              <?php }
              for($i = $from; $i <= $to; $i++) {
                if($i == $pageNum){ ?>
                  <a href='<?=ROOT_URL?>categories_post.php?id=<?=$id?>&pageNum=<?=$i?>' class="pagination-btn active"><?=$i?></a>
                <?php } else { ?>
                  <a href='categories_post.php?id=<?=$id?>&pageNum=<?=$i?>' class="pagination-btn"><?=$i?></a>
              <?php  }
              }
              if($pageNum < $totalRow){ ?>
              <a href='<?=ROOT_URL?>categories_post.php?id=<?=$id?>&pageNum=<?=$pageNext?>' class="pagination-btn" aria-label="next page">
              <i class="fa-solid fa-arrow-right"></i>
              </a>
              <a href='<?=ROOT_URL?>categories_post.php?id=<?=$id?>&pageNum=<?=$totalRow?>' class="pagination-btn" aria-label="next page">
